...
export default function IndexPage() {
  ...
  const handleSignIn = useMutation(
    async (options: GetSignInMutationOptions<UserCredentials>) =>
      signInMutation(options),
    {
      onSuccess: () => {
        // Invalidate session
  ...
  return (
    <div>
      ...
      <div>
        <pre>
          <code>{JSON.stringify({ sessionData, csrfData }, null, 3)}</code>
        </pre>
      </div>
    </div>
  );
